...
describe('Sources', () => {
  describe('canvas', () => {
    // Can't run in jsdom - requires real canvas pixel rendering
    it.skip('returns expected value', async () => {
      const { winding, text, geometry } = await getCanvasFingerprint()

      expect(winding).toBe(true)
      ...
      if (shouldSkip()) {
        ...
        expect(isDataURL(geometry)).toBe(true)
        expect(isDataURL(text)).toBe(true)
