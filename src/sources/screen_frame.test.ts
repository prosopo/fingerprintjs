...
describe('Sources', () => {
  describe('screenFrame', () => {
    ...
    // Can't run in jsdom - screen frame detection issues
    it.skip('gets non-zero frame', async () => {
    ...
    // Can't run in jsdom - screen frame detection issues
    it.skip('gets frame with unavailable position (IE and Edge ≤18)', async () => {
      await withMockProperties(
        screen,
        {
          width: { get: () => 1280 },
    ...
    // Can't run in jsdom - screen frame detection issues
    it.skip('watches for screen frame', async () => {
      try {
        vi.useFakeTimers()
        ...
        await withMockProperties(
          ...
          async () => {
            ...
            await vi.advanceTimersByTimeAsync(screenFrameCheckInterval)
          },
        )
